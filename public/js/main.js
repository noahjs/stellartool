jQuery(function ($) {
  ...
  /*
  var remote = stellar.Remote.from_config({
    "trace" : true,
    "websocket_ip" : "live.stellar.com",
    "websocket_port" : 9001,
    "websocket_ssl" : true
  });
  */
  remote.connect(function() {
    $('#submit-button').html('Get info');
  ...
  remote.once('connected', function () {
    ...
    if (stellar.UInt160.from_json(target).is_valid() ||
        reTxId.exec(target) ||
  ...
  function fetchTarget(target)
  {
    ...
    $("#graphlink").attr("href", "https://stellar.com/graph/#" + target);
    ...
    if (stellar.UInt160.from_json(target).is_valid()) {
      ...
      $("#datalink").attr("href", "https://stellar.com/client/#/tx/?id=" + target);
  ...
  $('.account-info-expand').click(function () {
    $("#account_info .expanded").removeClass("expanded");
    $("#account_info").find("ul > li").addClass("expanded");
  });

  $('.account-info-collapse').click(function () {
    $("#account_info .expanded").removeClass("expanded");
  });
  ...
  function handleError(err) {
    ...
    if ("string" === typeof err) {
      ...
      // Hide the results Groups
      $('.group').hide();

      if (err.error_message) {
        $("#error").show();
        $('.error-body').text(err.error_message);
      } else if (err.error) {
        $("#error").show();
        $('.error-body').text(err.error);
      } else {
        $("#error").show();
        $('.error-body').text(err.toString());
      }
    }
  }
